...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key in [pygame.K_UP, pygame.K_w] and direction != (0, CELL_SIZE):
                ...
                direction = (CELL_SIZE, 0)
            elif event.key == pygame.K_p: #key for PAUSE = P
                paused = not paused
                if paused: #Giving information in pause
                    res_cur.execute(
                        "INSERT INTO results (username, score, level, saved_at) VALUES (%s, %s, %s, %s)",
                        (username, score, level, datetime.now())
                    )
                    res_conn.commit()
                    print(f"Game on pause. Progress is saved: {username}, Score: {score}, Level: {level}")    
    ...
    if paused:
        pause_text = SCORE_FONT.render("Paused", True, (255, 255, 0))
        screen.blit(pause_text, (260, 180))
