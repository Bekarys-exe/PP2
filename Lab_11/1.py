...
def is_valid_phone(phone):
    return bool(phone) and all(ch in ALLOWED_PHONE_CHARS for ch in phone)
...
def get_contacts_by_pattern(pattern):
    ...
    cur.execute(
        "SELECT full_name, phone FROM phonebook "
        "WHERE full_name ILIKE %s OR phone LIKE %s ORDER BY full_name",
...
def upsert_contact(full_name, phone):
    conn = connect()
    cur = conn.cursor()
    cur.execute(
        "UPDATE phonebook SET phone = %s WHERE full_name = %s",
    ...
    if cur.rowcount > 0:
        conn.commit()
        print(f"Updated phone for {full_name} to {phone}.")
    else:
        try:
            cur.execute(
                "INSERT INTO phonebook (full_name, phone) VALUES (%s, %s)",
            ...
            conn.rollback()
            print(f"Error inserting/updating {full_name}: {e}")
    cur.close()
    conn.close()

def insert_many_contacts(contacts):
    invalid = []
    for full_name, phone in contacts:
        if is_valid_phone(phone):
            upsert_contact(full_name, phone)
        else:
            invalid.append((full_name, phone))
    if invalid:
        print("Invalid contacts (skipped):")
        for name, ph in invalid:
            print(f" - {name}: {ph}")
    return invalid

def query_contacts_paginated(limit, offset):
    conn = connect()
    cur = conn.cursor()
    cur.execute(
        "SELECT full_name, phone FROM phonebook "
    ...
    rows = cur.fetchall()
    print(f"No | Full Name           | Phone  (limit={limit}, offset={offset})")
    print("-----------------------------------------------------------------")
    for i, (full_name, phone) in enumerate(rows, start=offset+1):
        print(f"{i:<2} | {full_name:<20} | {phone}")
    cur.close()
    conn.close()
    return rows
...
def delete_contact(identifier):
    ...
    if is_valid_phone(identifier):
        ...
        print(f"Deleted contact with phone: {identifier}.")
    else:
        cur.execute("DELETE FROM phonebook WHERE full_name = %s", (identifier,))
        print(f"Deleted contact with full name: {identifier}.")
    conn.commit()
    cur.close()
    conn.close()
...
def print_menu():
    ...
    print("11. Query by pattern")
    print("12. Upsert contact")
    print("13. Insert many contacts")
    print("14. Query paginated")
    print("15. Delete by name or phone")
    print("0. Exit")
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            pat = input("Pattern for name or phone: ").strip()
            get_contacts_by_pattern(pat)
        elif choice == '12':
            name = input("Full name: ").strip()
            ph = input("Phone: ").strip()
            upsert_contact(name, ph)
        elif choice == '13':
            print("Enter contacts (full_name,phone), empty line to finish:")
            contacts = []
            while True:
                line = input().strip()
                if not line:
            ...
            lim = int(input("Limit: ").strip())
            off = int(input("Offset: ").strip())
            query_contacts_paginated(lim, off)
        elif choice == '15':
            ident = input("Full name or phone: ").strip()
